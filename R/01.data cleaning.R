...
height <- weight %>% 
  ...
  select(mrn, treatment_start_dt, height = vitals_value_height, 
         height_unit = vitals_units_in, height_date = vitals_dtm) %>% 
  ...
  filter(!is.na(height)) %>% 
  arrange(mrn, time_height_tx_days) %>% 
  distinct(mrn, .keep_all = TRUE)

weight <- weight %>% 
  filter(!is.na(vitals_value_weight)) %>% 
  select(mrn, treatment_start_dt, weight = vitals_value_weight, 
         weight_unit = vitals_units_lbs, weight_date = vitals_dtm, 
         time_weight_tx_days = days_between_vitals_dt_tx_dt) %>% 
  ...
  filter(!is.na(weight)) %>% 
  arrange(mrn, time_weight_tx_days) %>% 
  distinct(mrn, .keep_all = TRUE)
