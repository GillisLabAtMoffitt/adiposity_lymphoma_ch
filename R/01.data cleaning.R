...
height <- weight %>% 
  ...
  mutate(time_height_tx_days = case_when(
    height_date <= treatment_start_dt                   ~ interval(
      start = height_date, end = first_treatment_dt) /
      duration(n=1, unit = "days")
  )) %>% 
...
weight <- weight %>% 
  ...
  mutate(time_weight_tx_days = case_when(
    weight_date <= treatment_start_dt                   ~ interval(
      start = weight_date, end = first_treatment_dt) /
      duration(n=1, unit = "days")
  )) %>% 
...
lymphoma_data_petct <- lymphoma_data_petct %>% 
  ...
  mutate(time_petct_tx_days = interval(
      start = petct_scans_date, end = first_treatment_dt) /
      duration(n=1, unit = "days")
  ) %>% 
...
lymphoma_data_contrast <- lymphoma_data_contrast %>% 
  ...
  mutate(time_contrast_tx_days = interval(
      start = contrast_scans_date, end = first_treatment_dt) /
      duration(n=1, unit = "days")
  ) %>% 
